...
int BOOST_JOIN(bdb_compare_var_, Name)(DB *db, const DBT *dbt1, const DBT *dbt2)
{
	...
	Type x1 = nark::BOOST_JOIN(load_var_, Name)((const unsigned char*)dbt1->data, &end1);
	Type x2 = nark::BOOST_JOIN(load_var_, Name)((const unsigned char*)dbt2->data, &end2);
	if (x1 < x2) return -1;
	if (x2 < x1) return  1;
	return 0;
}
	...
	(DB *db, const DBT *dbt1, const DBT *dbt2, int n)
{
	...
	for (int i = 0; i != n; ++i)
	{
		Type x1 = nark::BOOST_JOIN(load_var_, Name)(p1, &p1);
		Type x2 = nark::BOOST_JOIN(load_var_, Name)(p2, &p2);
		if (x1 < x2) return -1;
		if (x2 < x1) return  1;
		if (p1 >= q1 || p2 >= q2)
